...
pub fn App() -> impl IntoView {
    ...
    Effect::new(move |_| {
        ...
        unsafe {
            ...
            let update_js: JsValue = update_callback.as_ref().unchecked_ref::<js_sys::Function>().into();
            let _ = Reflect::set(
                &window,
            ...
            let complete_js: JsValue = complete_callback.as_ref().unchecked_ref::<js_sys::Function>().into();
            let _ = Reflect::set(
                &window,
    ...
    let call_backend = |cmd: &str| {
        ...
        // 执行JavaScript代码 - 传入字符串引用而非JsValue
        let _ = eval(&js_code);
    };
    ...
    let toggle_timer = move |_| {
        ...
        let cmd = if running { "pause_timer" } else { "start_timer" };
        call_backend(cmd);
    };
            ...
            <div class="relative w-64 h-64 mb-8">
                ...
                <svg class="w-full h-full" viewBox="0 0 240 240">
                    ...
                    <circle 
                        ...
                        stroke-dasharray={format!("{}", 2.0 * PI * 100.0)}
